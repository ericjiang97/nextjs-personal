...
class WordPressApiService {
  static async getAllPosts(pageSize: number = 100, pageNumber: number = 1): Promise<ApiRequest<Posts>> {
  ...
  static async getSinglePost(slug: string): Promise<ApiRequest<Post>> {
    const response = await fetch(`${SITE_CONFIG.urls.WORDPRESS_URL}/wp-json/wp/v2/posts?slug=${slug}`);
    const blogPosts: Post[] = await response.json();
    if (blogPosts.length === 0) {
      return { error: { statusCode: 404 }, data: null };
    }
  ...
  static async getCategory(categoryId: string): Promise<ApiRequest<Category>> {
    const response = await fetch(`${SITE_CONFIG.urls.WORDPRESS_URL}/wp-json/wp/v2/categories/${categoryId}`);
    if (!response.ok) {
