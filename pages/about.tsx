  ...
  <PageLayout title="About Me">
    <MainContainer>
      <div className="max-w-4xl mx-auto py-auto pb-2 flex flex-col justify-around">
        <h1 className="m-0 w-full pt-14 leading-tight text-5xl text-center font-semibold">About Me</h1>
    ...
    <div className="max-w-4xl mx-auto pt-10 py-auto pb-8 flex flex-col flex-wrap justify-around">
      <h2 className="m-0 w-full pt-14 leading-tight text-xl text-left font-bold">Experience</h2>
      {experience.map((experience: Experience, index) => {
        return (
          <div className="max-w-4xl py-auto flex flex-row flex-wrap justify-around py-6 px-4 flex-1" key={index}>
            <div className="w-1/3 mx-auto py-auto flex flex-1 flex-col justify-start">
              <a href={experience.companyUrl} target="_blank" rel="noreferrer noopener" className="text-left underline">
                <h3 className="w-full leading-tight text-lg text-left font-semibold">{experience.company}</h3>
              </a>
              <div className="px-4 py-2">
                {experience.positions.map((position, posIndex) => (
                  <div key={posIndex}>
                    <span className="text-xs">{position.dates}</span>
                    <h4 className="w-full leading-tight text-md text-left mb-2">{position.title}</h4>
                  </div>
                ))}
              </div>
            </div>
            <div className="max-w-2xl mx-auto py-auto ml-4 flex flex-3 flex-col" style={{ minWidth: 280 }}>
